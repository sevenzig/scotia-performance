 ...
 * @param {string} media - Media query for the CSS (default: 'all')
 * @param {boolean} preload - Whether to use preload strategy (default: true)
 * @returns {HTMLLinkElement} - The created link element
 */
export function loadCSS(href, media = 'all', preload = true) {
 ...
 * @returns {void}
 */
export function loadFont(fontUrl, fontFamily, fontWeight = 'normal', fontStyle = 'normal') {
 ...
 * @param {number} rootMargin - Margin around the root (in pixels, default: 200)
 * @returns {void}
 */
export function loadCSSWhenVisible(href, selector, rootMargin = 200) {
 ...
 * @param {string} css - The CSS string to extract critical styles from
 ...
 */
export function optimizeStylesheetLoading() {
  if (typeof document === 'undefined') return;
